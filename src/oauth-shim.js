...
oauth_shim.init = function (arr) {
	console.debug('oauth_shim.init called');
	console.debug(arr);
...
oauth_shim.request = function (req, res, next) {
	console.debug('oauth_shim.request called');
	const self = oauth_shim;
...
oauth_shim.interpret = function (req, res, next) {
	console.debug('oauth_shim.interpret called');
	...
	if ((p.code || p.refresh_token) && p.redirect_uri) {
		...
		p.redirect_uri &&
		((p.oauth && parseInt(p.oauth.version, 10) === 1) || p.oauth_token)
	) {
		// OAUTH1
		// Credentials...
		login(
...
oauth_shim.proxy = function (req, res, next) {
	...
	if (p.access_token && p.path) {
		...
		signRequest(
			...
			p.data,
			p.access_token,
			proxyHandler.bind(null, req, res, next, p, buffer)
		);
	} else if (p.path) {
...
oauth_shim.unhandled = function (req, res) {
	...
	serveUp(
		res,
		errorObj("invalid_request", "The request is unrecognised"),
		p.callback
	);
};
